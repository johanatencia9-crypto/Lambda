...
```bash
LambdaRPA$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
LambdaRPA$ sam local start-api
LambdaRPA$ curl http://localhost:3000/
```
...
```bash
LambdaRPA$ sam logs -n HelloWorldFunction --stack-name LambdaRPA --tail
```
...
```bash
LambdaRPA$ cd HelloWorldFunction
...
```bash
sam delete --stack-name LambdaRPA
```
